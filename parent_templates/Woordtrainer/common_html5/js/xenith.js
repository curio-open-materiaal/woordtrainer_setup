...
var x_languageData = [],
	...
	// Auto-generation system for word list exercises
	x_autoGenerateMode = false,  // flag to indicate if auto-generation is active
	x_virtualPages = [],  // array for generated virtual pages
	x_wordListData = [],  // parsed word list data
	x_virtualIntroPage = null,  // virtual introPage if no real introPage exists
	x_btnIcons = [
...
$(document).ready(function () {
	...
	if (typeof dataxmlstr != 'undefined') {
		...
		let url = "website_code/php/templates/get_template_xml.php?file=" + encodeURIComponent(x_projectXML) + "&time=" + now;
		if (typeof use_url !== "undefined" && use_url) {
			url = x_projectXML + "?time=" + now;
		}
		var xmlLoadAttempt = 0;
		var maxXmlLoadAttempts = 2;
		var xmlLoadTimeoutMs = 25000;
		...
		function doLoadXml() {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					if (xmlLoadAttempt < maxXmlLoadAttempts && (textStatus === "timeout" || textStatus === "error")) {
						doLoadXml();
						return;
					}
					var msg = textStatus === "timeout"
						? "De projectgegevens zijn niet in tijd geladen. Vernieuw de pagina om het opnieuw te proberen."
						: "De projectgegevens zijn niet geladen.";
					$("body").append("<p>" + msg + "</p>");
				}
			});
		}
...
x_projectDataLoaded = function (xmlData) {
	...
	// Check if auto-generation should be activated (only word list exists)
	x_detectAutoGenerate();
...
function x_setUp() {
	...
	if (x_normalPages.length == 0) {
		...
		$x_headerBlock.prepend('<div id="x_headerIcon" style="font-size: 2.5rem; color: rgba(255,255,225, 1); position: absolute; left: 300px; top: 45px;"></div>');
		
		// Add mobile navbar CSS
		$x_head.append('<style id="mobile_navbar_css">' +
			'@media (max-width: 768px) {' +
				...
				'}' +
			'}' +
			/* Darker nav buttons in footer */
			'#x_footerBlock .x_floatLeft button, #x_footerBlock .x_floatRight button { background-color: #4a148c !important; color: #fff !important; border-color: #3a0f6d !important; }' +
			'#x_footerBlock .x_floatLeft button:hover, #x_footerBlock .x_floatRight button:hover { background-color: #3a0f6d !important; }' +
			'#x_footerBlock .x_floatLeft button:disabled, #x_footerBlock .x_floatRight button:disabled { background-color: #7b5aa6 !important; opacity: 0.7; }' +
			/* Make all titles bold */
			'.titlePageTxt, .titles h3, .level-title, h1, h2, h3.title { font-weight: bold !important; }' +
			/* Reset position for exercise pages - only title and introPage should use absolute positioning */
			'#pageContents:not([data-position-set-by-page="true"]) { position: relative !important; top: auto !important; left: auto !important; transform: none !important; }' +
		'</style>');
...
function x_continueSetUp1() {
	if (setUpComplete == false) {
		...
		for (let i = 0; i < x_pages.length; i++) {
			// Skip virtual pages or null/undefined pages
			if (x_pages[i] && x_pages[i] !== null && (typeof x_pages[i].getAttribute === 'function')) {
		...
		$x_nextBtn
			...
			.click(function () {
				// Special handling for woordenlijst page - navigate to introPage (level selection)
				var currentPageType = x_pageInfo[x_currentPage] ? x_pageInfo[x_currentPage].type : '';
				var pageXML = x_pages[x_currentPage];
				var tagName = '';
				if (pageXML && typeof pageXML.tagName !== 'undefined') {
					tagName = pageXML.tagName.toLowerCase();
				}
				var isWoordenlijst = currentPageType === 'woordenlijstextended' || currentPageType === 'woordenlijst extended' || 
				                     currentPageType === 'woordenlijstcustom' || currentPageType === 'woordenlijst custom' ||
				                     tagName === 'woordenlijstextended' || tagName === 'woordenlijstcustom' || tagName === 'woordenlijst';
				
				if (isWoordenlijst) {
					// Find title page (level selection page)
					for (var i = 0; i < x_pageInfo.length; i++) {
						var pageType = x_pageInfo[i].type ? x_pageInfo[i].type.toLowerCase() : '';
						var checkPageXML = x_pages[i];
						var checkTagName = '';
						if (checkPageXML && typeof checkPageXML.tagName !== 'undefined') {
							checkTagName = checkPageXML.tagName.toLowerCase();
						}
						
						// Check for title page (level selection)
						if (pageType === 'title' || checkTagName === 'title') {
							x_changePage(i);
							$(this).removeClass("ui-state-focus").removeClass("ui-state-hover");
							return;
						}
					}
					// If title page not found, fall through to normal navigation
				}
		...
		if ($x_menuBtn.length > 0) {
			...
			let menuLabel = x_params.tocLabel != undefined && x_params.tocLabel != "" ? x_params.tocLabel : x_getLangInfo(x_languageData.find("tocButton")[0], "label", "Inhoudsopgave");
			...
			$x_menuBtn
				...
				.click(function () {
					try {
						...
						var targetIndex = 0;
						for (var i = 0; i < x_pageInfo.length; i++) {
							if (x_pageInfo[i].standalone != true && x_pageInfo[i].type == 'title') { targetIndex = i; break; }
						}
...
function x_getIntroInfo(xml) {
	...
	if (xml == undefined || xml == null || xml == 'menu' || (typeof xml !== 'string' && typeof xml.getAttribute !== 'function')) {
		if (xml == 'menu') {
			return false;
		}
		return false;
...
function x_checkPages(type, id, pageArray) {
	...
	var checkChildIDs = function (ids) {
		// Ensure ids is defined and is an array
		if (ids == null || !$.isArray(ids)) {
			return false;
		}
	...
	for (var i = 0; i < pageArray.length; i++) {
		...
		if (pageArray[i].type != 'menu') {
			var childIDs = pageArray[i].childIDs;
			// Ensure childIDs is defined and is an array
			if (childIDs == null || !$.isArray(childIDs)) {
				childIDs = [];
			}
			var result = checkChildIDs(childIDs);
			if (result === true) {
				return tempArray;
				break;
			}
...
function x_changePage(x_gotoPage, addHistory) {
	...
	if ((x_gotoPage == 0 && XENITH.PAGEMENU.menuPage) || !standAlonePage || x_pages[x_gotoPage].getAttribute('linkTarget') == 'same' || x_firstLoad) {
		if ($x_body.width() == 0 && $x_body.height() == 0) {
			...
			// Skip CSS loading for virtual pages (they don't have CSS files)
			var isVirtualPage = x_pageInfo[x_gotoPage] && x_pageInfo[x_gotoPage].virtual === true;
			
			// List of page types that have CSS embedded in HTML or don't have CSS files
			var pagesWithoutCSS = ['woordenLijstExtended', 'woordenlijstextended', 'woordenLijstExtendedCustom', 'woordenlijstextendedcustom', 'woordenLijstCustom', 'woordenlijstcustom'];
			
			if (isVirtualPage || pagesWithoutCSS.indexOf(modelfile) !== -1) {
				// Virtual pages or pages without separate CSS files don't need CSS loading
	...
	if (x_currentPage != -1 && !XENITH.PAGEMENU.isThisMenu() && (!pagechange || x_currentPage != x_gotoPage) && x_pageInfo[x_currentPage].passwordPass != false) {
		...
		if (x_pageInfo[x_currentPage].type == "text") {
			...
			try {
				...
				// If the page object doesn't exist yet (e.g., for virtual pages that haven't loaded),
				// create a minimal object to prevent errors
				pageObj = null;
			}
		}
		if (pageObj && typeof pageObj.leavePage === 'function') {
			pageObj.leavePage();
		}
...
function x_changePageStep2(x_gotoPage) {
	...
	var isVirtualPage = x_pageInfo[x_gotoPage] && x_pageInfo[x_gotoPage].virtual === true;
	if (isVirtualPage && x_autoGenerateMode) {
		// Check if this is the virtual introPage (at index 0)
		if (x_gotoPage === 0 && x_pageInfo[x_gotoPage].isVirtualIntro === true) {
			// Virtual introPage - create XML element for introPage.html
			var tempXML = document.createElement('introPage');
			tempXML.setAttribute('name', 'introPage');
			tempXML.setAttribute('virtual', 'true');
			// Get woordenSet from word list page or use default
			var woordenSet = '';
			if (typeof x_wordListData !== 'undefined' && x_wordListData.length > 0) {
				woordenSet = 'Woordenset';
			}
			tempXML.setAttribute('woordenSet', woordenSet);
			
			// Store in x_pages array for this virtual page
			if (!x_pages[x_gotoPage] || x_pages[x_gotoPage] === null) {
				while (x_pages.length <= x_gotoPage) {
					var tempPagesArray = Array.from(x_pages);
					tempPagesArray.push(null);
					x_pages = $(tempPagesArray);
				}
			}
			// Update the XML in x_pages array
			var tempPagesArray = Array.from(x_pages);
			tempPagesArray[x_gotoPage] = tempXML;
			x_pages = $(tempPagesArray);
			x_currentPageXML = tempXML;
		} else {
			// Calculate virtual page index (accounting for existing pages before virtual ones)
			// Find where virtual exercise pages start (after introPage, title, woordenlijst)
			// Virtual introPage (if exists) is NOT in x_virtualPages array, it's separately in x_pageInfo
			var baseIndex = -1;
			
			// Find the first virtual exercise page (not virtual introPage)
			for (var j = 0; j < x_pageInfo.length; j++) {
				if (x_pageInfo[j].virtual && !x_pageInfo[j].isVirtualIntro) {
					baseIndex = j;
					break;
				}
			}
			
			// If we found where virtual exercises start, use that
			if (baseIndex >= 0) {
				var virtualIndex = x_gotoPage - baseIndex;
				
				
				if (virtualIndex >= 0 && virtualIndex < x_virtualPages.length) {
					var virtualPageData = x_virtualPages[virtualIndex];
					if (virtualPageData) {
						var tempXML;
						try {
							// Create temporary XML element for virtual page
							tempXML = document.createElement(virtualPageData.type);
							tempXML.setAttribute('name', virtualPageData.type);
							tempXML.setAttribute('virtual', 'true');
							
							// Special handling for flashCards - use newFlashCard styling
							if (virtualPageData.type === 'flashCards' && virtualPageData.config) {
								// Change type to newFlashCard for proper styling
								tempXML = document.createElement('newFlashCard');
								tempXML.setAttribute('name', 'newFlashCard');
								tempXML.setAttribute('virtual', 'true');
								
								var word = virtualPageData.config.word || '';
								var betekenis = virtualPageData.config.betekenis || '';
								
								// Set text attribute (introduction) for newFlashCard - use default instruction
								tempXML.setAttribute('text', 'Draai de kaart om om de betekenis van het woord te zien.');
								
								// newFlashCard uses sideA (front) and sideB (back) instead of child card elements
								if (word) {
									tempXML.setAttribute('sideA', String(x_stripHtml(word)));
								}
								if (betekenis) {
									tempXML.setAttribute('sideB', String(x_stripHtml(betekenis)));
								}
							} else if (virtualPageData.type === 'woordtrainerQuizSpelling' && virtualPageData.config) {
								// woordtrainerQuizSpelling uses woordspellingAudio.html file
								// Change type to woordspellingAudio so it loads the correct HTML file
								tempXML = document.createElement('woordspellingAudio');
								tempXML.setAttribute('name', 'woordspellingAudio');
								tempXML.setAttribute('virtual', 'true');
								
								// Set attributes from config
								if (virtualPageData.config) {
									for (var key in virtualPageData.config) {
										if (virtualPageData.config.hasOwnProperty(key)) {
											tempXML.setAttribute(key, String(virtualPageData.config[key]));
										}
									}
								}
							} else if (virtualPageData.config) {
								// Set attributes based on config for other exercise types
								for (var key in virtualPageData.config) {
									if (virtualPageData.config.hasOwnProperty(key)) {
										tempXML.setAttribute(key, String(virtualPageData.config[key]));
									}
							...
							console.error('Error creating virtual page XML:', e, virtualPageData);
							// Fallback: create minimal XML element
							tempXML = document.createElement(virtualPageData.type || 'text');
							tempXML.setAttribute('name', virtualPageData.type || 'text');
						}
						
						// Store in x_pages array for this virtual page
						if (!x_pages[x_gotoPage] || x_pages[x_gotoPage] === null) {
							// Need to expand x_pages array
							while (x_pages.length <= x_gotoPage) {
								x_pages.push(null);
							}
						}
						x_pages[x_gotoPage] = tempXML;
						x_currentPageXML = tempXML;
					} else {
						console.error('Virtual page data is null for index:', virtualIndex);
						x_currentPageXML = x_pages[x_currentPage];
					}
				} else {
					console.error('Virtual page index out of range:', virtualIndex, 'x_virtualPages.length:', x_virtualPages.length);
					x_currentPageXML = x_pages[x_currentPage];
				}
			} else {
				console.error('Could not find baseIndex for virtual exercises');
				x_currentPageXML = x_pages[x_currentPage];
			}
		}
	} else {
	...
	let chapterIndex = x_currentPageXML.getAttribute("chapterIndex");
	let baseTitle = x_params['name'];
    // Track progress milestones around title pages and end of LO
    try {
        var titleIndices = [];
        for (var i = 0; i < x_pageInfo.length; i++) {
            if (x_pageInfo[i].standalone != true && x_pageInfo[i].type == 'title') { titleIndices.push(i); }
        }

        var progressKey = 'xerte_progress_' + (typeof x_TemplateId !== 'undefined' ? x_TemplateId : 'default');
        var progress = {};
        try { progress = JSON.parse(localStorage.getItem(progressKey) || '{}') || {}; } catch(e) { progress = {}; }

        if (titleIndices.length > 0 && x_currentPage === titleIndices[0] - 1) {
            progress.seenBeforeTitle1 = true;
        }
        if (titleIndices.length > 1 && x_currentPage === titleIndices[1] - 1) {
            progress.seenBeforeTitle2 = true;
        }
        if (titleIndices.length > 2 && x_currentPage === titleIndices[2] - 1) {
            progress.seenBeforeTitle3 = true;
        }
        // mark when reaching the very last page of the LO (non-standalone)
        var lastIndex = -1;
        for (var j = x_pageInfo.length - 1; j >= 0; j--) {
            if (x_pageInfo[j].standalone != true) { lastIndex = j; break; }
        }
        if (lastIndex !== -1 && x_currentPage === lastIndex) {
            progress.seenEnd = true;
        }

        localStorage.setItem(progressKey, JSON.stringify(progress));
        try {
            window.dispatchEvent(new CustomEvent('x_progress_updated', { detail: { key: progressKey, progress: progress } }));
        } catch(e) {}
        // When starting a new level (first exercise of that level), clear current-level stats
        if (typeof x_getCurrentLevel === 'function' && typeof x_getLevelStartIndex === 'function') {
            var currentLevel = x_getCurrentLevel(x_currentPage);
            var levelStartIdx = currentLevel != null ? x_getLevelStartIndex(currentLevel) : null;
            if (levelStartIdx !== null && x_currentPage === levelStartIdx) {
                try {
                    var currentLevelKey = 'wt:currentLevelStats:' + (location.pathname || '') + (location.search || '');
                    localStorage.setItem(currentLevelKey, '{}');
                } catch (e) {}
            }
        }
    } catch(e) {}
	// change icons based of the page type
	// Remove any existing title translations in header
	$('#x_headerBlock .title-translation').remove(); 
	var pageName = (x_currentPageXML.getAttribute('name') || '').toLowerCase();

	// Treat both woordenlijst and woordenlijstextended the same
	if (pageName === "woordenlijst" || pageName === "woordenlijstextended" || pageName === "woordenlijst extended") {
		// show icon absolute positioned in header left 
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-grip');
		$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">woordenlijst</span>');
	} else if (pageName.includes("context")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-comment-alt');
		$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">contextzin</span>');
	} 
	else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('spelling audio')) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-play'); 
		$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">hoor het woord</span>');
	 } 
	else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("intro") || x_currentPageXML.getAttribute('name')?.toLowerCase().includes("afgerond")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-info-circle');
		$('.inserted-label').remove();
		// if afgerond
		if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("level")) {
			$('#x_headerBlock h1').after('<span class="inserted-label">kies niveau</span>');
		} else {
			$('#x_headerBlock h1').after('<span class="inserted-label">start</span>');
		}
	} else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("herhaal")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-microphone-lines');
			$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">spreek en vergelijk</span>');
	} else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("flash")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-arrows-rotate');
			$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">flitskaart</span>');
	} else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("welke zin")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-question-circle');
			$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">welke zin is juist?</span>');
	} else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('meerkeuzevraag') || x_currentPageXML.nodeName === 'meerkeuzevraag') {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-list-check');
		$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">meerkeuzevraag</span>');
	} else if( x_currentPageXML.getAttribute('name')?.toLowerCase().includes("quiz") || x_currentPageXML.getAttribute('name')?.toLowerCase().includes("test")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-question');
			$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">meerkeuzevraag</span>');
	} else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("associaties")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-arrows-turn-to-dots');
		$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">wat hoort bij wat?</span>');
	} else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("categorie")) {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-hand-pointer'); 
			$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">sorteer de woorden</span>');
	} else if (x_currentPageXML.getAttribute('name')?.toLowerCase().includes("memory") || x_currentPageXML.nodeName === 'woordMemory') {
		$('#x_headerIcon').removeAttr('class').addClass('fa fa-memory');
		$('.inserted-label').remove();
		$('#x_headerBlock h1').after('<span class="inserted-label">woord memory</span>');
	}
	 else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('woordspelling')) {
			$('#x_headerIcon').removeAttr('class').addClass('fa fa-spell-check'); 
			$('.inserted-label').remove();
			$('#x_headerBlock h1').after('<span class="inserted-label">woordspelling</span>');
	 } 
	 else if((x_currentPageXML.getAttribute('name')?.toLowerCase().includes('vul') && x_currentPageXML.getAttribute('name')?.toLowerCase().includes('zinnen')) || x_currentPageXML.getAttribute('name')?.toLowerCase().includes('fillblanks') || x_currentPageXML.nodeName?.toLowerCase() === 'woordtrainerfillblanks') {
			$('#x_headerIcon').removeAttr('class').addClass('fa fa-minus'); 
			$('.inserted-label').remove();
			$('#x_headerBlock h1').after('<span class="inserted-label">vulzinnen</span>');
	 } 
	 else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('woordverdwijner') || x_currentPageXML.nodeName === 'woordverdwijner') {
			$('#x_headerIcon').removeAttr('class').addClass('fa fa-ghost'); 
			$('.inserted-label').remove();
			$('#x_headerBlock h1').after('<span class="inserted-label">woordverdwijner</span>');
	 }
	 else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('woordshuffle') || x_currentPageXML.nodeName === 'woordshuffle') {
			$('#x_headerIcon').removeAttr('class').addClass('fa fa-shuffle'); 
			$('.inserted-label').remove();
			$('#x_headerBlock h1').after('<span class="inserted-label">woord shuffle</span>');
	 }
	 else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('woordzoeker') || x_currentPageXML.nodeName?.toLowerCase() === 'woordzoeker') {
			$('#x_headerIcon').removeAttr('class').addClass('fa fa-magnifying-glass');
			$('.inserted-label').remove();
			$('#x_headerBlock h1').after('<span class="inserted-label">woordzoeker</span>');
	 }
	 else if(x_currentPageXML.getAttribute('name')?.toLowerCase().includes('beeld') && x_currentPageXML.getAttribute('name')?.toLowerCase().includes('woord') || x_currentPageXML.nodeName?.toLowerCase() === 'beeldnaarwoord') {
			$('#x_headerIcon').removeAttr('class').addClass('fa fa-image');
			$('.inserted-label').remove();
			$('#x_headerBlock h1').after('<span class="inserted-label">beeld naar woord</span>');
	 }
	else {
		$('#x_headerIcon').removeAttr('class');
		$('.inserted-label').remove();
	}
	...
	// Check if we're in auto-generate mode and show level in title
	var levelText = '';
	if (x_autoGenerateMode && typeof x_currentPage !== 'undefined') {
		var currentLevel = x_getCurrentLevel(x_currentPage);
	...
	if (chapterIndex != undefined && x_chapters[chapterIndex]?.includeInHeader == "true") {
		let headerName = x_chapters[chapterIndex].headerName || x_chapters[chapterIndex].name;
		
		$("#x_headerBlock h1").html(baseTitle + levelText + ": " + headerName);
	} else {
		$("#x_headerBlock h1").html(baseTitle + levelText);
	}
	...
	if (XENITH.PAGEMENU.isThisMenu()) {
		pageTitle = x_getLangInfo(x_languageData.find("toc")[0], "label", "Inhoudsopgave");
...
function x_changePageStep3() {
	...
	if (x_pageInfo[x_currentPage].built != false) {
		...
		// Remove all previous page content before appending new page
		$x_pageDiv.children().remove();
		...
		function loadModel() {
			// Remove all previous page content before loading new page
			$x_pageDiv.children().remove();
...
function x_setUpPage() {
	...
	// Reset position for exercise pages (not title or introPage)
	var currentPageType = x_pageInfo[x_currentPage] ? x_pageInfo[x_currentPage].type : '';
	var isTitleOrIntro = currentPageType === 'title' || currentPageType === 'intropage';
	if (!isTitleOrIntro && $('#pageContents').length > 0) {
		// Only reset if not already set by the page's own positioning logic
		var positionSetByPage = $('#pageContents').attr('data-position-set-by-page');
		if (positionSetByPage !== 'true') {
			$('#pageContents').css({
				'position': 'relative',
				'top': 'auto',
				'left': 'auto',
				'transform': 'none'
			}).attr('data-position-set-by-page', 'false');
		}
	}
	...
	if (pageIndex != -1 && pageIndex < x_normalPages.length - 1) {
		// Check if this is an exercise page that needs completion before continuing
		var shouldCheckCompletion = false;
		var isExercisePage = false;
		
		// Get current page type (raw + lowercase)
		var currentPageTypeRaw = '';
		var currentPageType = '';
		if (x_pageInfo[x_currentPage] && x_pageInfo[x_currentPage].type) {
			currentPageTypeRaw = x_pageInfo[x_currentPage].type;
			currentPageType = currentPageTypeRaw.toLowerCase();
		}
		
		// Define non-exercise page types that should always allow navigation
		// Note: use lowercase values here
		var nonExerciseTypes = [
			'intropage',
			'title',
			'woordenlijst',
			'woordenlijstextended',
			'woordenlijst extended',
			'woordenlijstcustom',
			'woordenlijst custom',
			'levelcompletion',
			'colofon',
		...
		// Types that should never require completion (flashcards + audio recording)
		// Note: these are checked against lowercase currentPageType
		var skipTypes = ['newflashcard', 'flashcards', 'herhaaldezin', 'herhaal de zin']; // Always allow navigation for these
		
		// Also check raw type for case variations (to catch any casing like herhaalDeZin, herhaaldezin, etc.)
		var isHerhaalDeZin = currentPageTypeRaw && currentPageTypeRaw.toLowerCase().indexOf('herhaal') !== -1;
		
		// Check if this is an exercise page (not a special page)
		if (currentPageType && nonExerciseTypes.indexOf(currentPageType) === -1) {
			isExercisePage = true;
			// Check if we should skip completion check for this type
			// Also check if it's herhaalDeZin regardless of exact case
			if (skipTypes.indexOf(currentPageType) === -1 && !isHerhaalDeZin) {
				shouldCheckCompletion = true;
			}
		}
		
		// Always enable the "verder" button - no restrictions
		if ($x_nextBtn.length > 0) {
			if ($x_nextBtn.data("ui-button")) {
		...
		// Also always enable top button
		var $nextTop = $("#x_nextBtn_top");
		if ($nextTop.length > 0) {
			if ($nextTop.data("ui-button")) {
				$nextTop.button("enable");
			} else {
				$nextTop
					.removeClass("ui-state-disabled")
					.prop("disabled", false);
...
// Listen for exercise completion events to check level completion
if (typeof window !== 'undefined') {
	// Listen for exercise completion to enable "verder" button
	window.addEventListener('wt:exerciseDone', function(event) {
		try {
			var exerciseId = event.detail ? event.detail.exerciseId : '';
			if (!exerciseId) return;
			
			// Get current page type to check if this completion is for the current page
			var currentPageType = '';
			if (x_pageInfo[x_currentPage] && x_pageInfo[x_currentPage].type) {
				currentPageType = x_pageInfo[x_currentPage].type.toLowerCase();
			}
			
			// Only enable if this exercise matches the current page
			if (exerciseId.toLowerCase() === currentPageType) {
				// Check if we're still on the same page and if the button should be enabled
				var pageIndex = $.inArray(x_currentPage, x_normalPages);
				if (pageIndex != -1 && pageIndex < x_normalPages.length - 1) {
					if ($x_nextBtn.length > 0 && $x_nextBtn.data("ui-button")) {
						$x_nextBtn.button("enable");
					} else if ($x_nextBtn.length > 0) {
						$x_nextBtn.removeClass("ui-state-disabled").prop("disabled", false);
					}
					// Also enable top button
					var $nextTop = $("#x_nextBtn_top");
					if ($nextTop.length > 0 && $nextTop.data("ui-button")) {
						$nextTop.button("enable");
					} else if ($nextTop.length > 0) {
						$nextTop.removeClass("ui-state-disabled").prop("disabled", false);
					}
				}
			}
		} catch(e) {
	...
	window.addEventListener('wt:statsUpdated', function() {
		if (x_autoGenerateMode) {
			// Delay check slightly to ensure localStorage is updated
			setTimeout(function() {
				x_checkLevelCompletionOnExerciseComplete();
			}, 500);
		}
	}, { passive: true });

	// Record a word result for the current level only (used by progress modal "resultaten dit level")
	window.wtRecordCurrentLevelWordResult = function(word, isRight) {
		try {
			if (!word || typeof word !== 'string') return;
			var key = 'wt:currentLevelStats:' + (location.pathname || '') + (location.search || '');
			var data = {};
			try { data = JSON.parse(localStorage.getItem(key) || '{}'); } catch (e) {}
			var k = word.trim().toLowerCase();
			if (!k) return;
			if (!data[k]) data[k] = { word: word.trim(), right: 0, wrong: 0 };
			if (isRight) data[k].right += 1; else data[k].wrong += 1;
			localStorage.setItem(key, JSON.stringify(data));
		} catch (e) {}
	};
	
	// Also listen for page changes to check completion
	window.addEventListener('x_page_loaded', function() {
		if (x_autoGenerateMode) {
			setTimeout(function() {
				x_checkLevelCompletionOnPageChange();
			}, 1000);
		}
	}, { passive: true });
	
	// Listen for custom word set updates to trigger auto-generation if needed
	window.addEventListener('wt:customWordSetUpdated', function(event) {
		try {
			// Only trigger if auto-generation hasn't been activated yet
			if (typeof x_autoGenerateMode === 'undefined' || !x_autoGenerateMode) {
				// Check if we have a woordenlijstCustom page
				var hasCustomPage = false;
				var wordListPageIndex = -1;
				var titlePageIndex = -1;
				var introPageIndex = -1;
				
				for (var i = 0; i < x_pageInfo.length; i++) {
					var pageType = x_pageInfo[i] ? x_pageInfo[i].type : '';
					...
					if (pageType === 'woordenlijstcustom' || pageType === 'woordenlijst custom' || 
					    tagName === 'woordenlijstcustom') {
						hasCustomPage = true;
						wordListPageIndex = i;
					}
					
					if (pageType === 'title' || tagName === 'title') {
						titlePageIndex = i;
					}
					
					var isIntroType = pageType === 'intropage' || pageType === 'intro' || pageType === 'introduction';
					var isIntroTag = tagName === 'intropage' || tagName === 'intro' || tagName === 'introduction';
					if (isIntroType || isIntroTag) {
						introPageIndex = i;
					}
				}
				
				// Only proceed if we have a custom page and meet auto-generation conditions
				if (hasCustomPage && x_pageInfo.length <= 3) {
					// Check if we have words in localStorage now
					var wordListData = x_getWordListData(wordListPageIndex);
					if (wordListData.length > 0) {
						// Activate auto-generation
						x_wordListData = wordListData;
						x_autoGenerateMode = true;
						
						// Generate exercises
						x_generateExercises();
						
						// Ensure correct order
						x_ensureCorrectOrder(introPageIndex, wordListPageIndex, titlePageIndex);
						
						// Reload current page to show updated navigation
						if (typeof x_changePage === 'function') {
							var currentPage = x_currentPage;
							setTimeout(function() {
								x_changePage(currentPage);
							}, 100);
						}
					}
				}
			...
			console.error('Error handling custom word set update:', e);
		}
	}, { passive: true });
}

function x_checkLevelCompletionOnExerciseComplete() {
	if (!x_autoGenerateMode || !x_virtualPages || x_virtualPages.length === 0) {
		return;
	}

	try {
		var baseIndex = x_pageInfo.length - x_virtualPages.length;
		var currentVirtualIndex = x_currentPage - baseIndex;
		
		if (currentVirtualIndex < 0 || currentVirtualIndex >= x_virtualPages.length) {
			return;
		}

		var currentVirtualPage = x_virtualPages[currentVirtualIndex];
		if (!currentVirtualPage || !currentVirtualPage.level) {
			return;
		}

		// Find the completion page for this level
		for (var i = 0; i < x_virtualPages.length; i++) {
			var virtualPage = x_virtualPages[i];
			if (virtualPage.type === 'levelCompletion' && 
			    virtualPage.config && 
			    virtualPage.config.level === currentVirtualPage.level) {
				
				var levelStartIndex = virtualPage.config.levelStartIndex;
				var levelEndIndex = virtualPage.config.levelEndIndex;
				
				// Check if this is the last exercise of the level
				if (currentVirtualIndex === levelEndIndex) {
					// Check if all exercises are completed
					var completedCount = x_countCompletedExercisesInLevel(levelStartIndex, levelEndIndex);
					var totalExercises = levelEndIndex - levelStartIndex + 1;
					
					// If all exercises completed, navigate to completion page
					if (completedCount >= totalExercises) {
						var completionPageIndex = baseIndex + i;
						setTimeout(function() {
							x_changePage(completionPageIndex);
		...
		console.error('Error checking level completion:', e);
	}
}

function x_checkLevelCompletionOnPageChange() {
	// Check if we're navigating to the next exercise after completing the last one
	x_checkLevelCompletionOnExerciseComplete();
}

// Helper: add translation under an element (used by individual HTML files)
function x_addTranslationUnder(element, text) {
	try {
		// Read selected language
		var targetLang = '';
		try {
			targetLang = localStorage.getItem('woordenlijst_selected_language') || '';
		} catch (e) {
			targetLang = '';
		}
		
		if (!targetLang || !targetLang.trim() || targetLang.toLowerCase() === 'nl') {
			return; // No translation needed
		}
		
		// If no text provided, get it from the element
		if (!text) {
			text = $(element).text() || '';
		}
		text = $.trim(text);
		...
		var $el = $(element);
		if ($el.next('.intro-translation').length > 0) {
			return; // Already has translation
		}
		
		// Create placeholder for translation
		var $translation = $('<div class="intro-translation" style="font-size: 0.8rem; color: #999; font-style: italic; margin-top: 0.05rem; text-align: center;"></div>');
		$el.after($translation);
		
		// Call MyMemory Translation API
		var url = 'https://api.mymemory.translated.net/get?q=' +
			encodeURIComponent(text) + '&langpair=nl|' + encodeURIComponent(targetLang) + '&de=f.vangils@curio.nl';
		...
		fetch(url)
			...
			.then(function (data) {
				try {
					...
					// MyMemory limit message: replace with short fallback
					if (translated && /MYMEMORY WARNING|USED ALL AVAILABLE FREE|NEXT AVAILABLE IN/i.test(translated)) {
						translated = 'Vertaling niet beschikbaar';
					}
					if (translated && $.trim(translated) && translated.toLowerCase() !== text.toLowerCase()) {
						// Remove duplicate trailing periods and colons (e.g. ".." -> ".", "::" -> ":")
						translated = $.trim(translated).replace(/\.{2,}$/g, '.').replace(/:{2,}$/g, ':');
...
function x_pageLoaded() {
	...
	// Translation is now handled per HTML file directly after intro text is set
}
...
function x_addNarration(funct, arguments) {
	...
	// Update custom top progress bar (header) for Woordtrainer theme
	if (typeof x_updateTopProgressBar === 'function') {
		try {
			x_updateTopProgressBar();
		} catch (e) {
			// Fail silently so navigation is not broken
		}
	}
}

// Level-based top progress bar for Woordtrainer theme
// Uses #x_progressBar and #x_progressBarWrapper created by woordtrainer.js
function x_updateTopProgressBar() {
	try {
		var $bar = $("#x_progressBar");
		var $wrapper = $("#x_progressBarWrapper");
		var $nextTop = $("#x_nextBtn_top");

		if ($bar.length === 0 || $wrapper.length === 0) {
			try { console.log('x_updateTopProgressBar: early exit because progress bar DOM missing', {bar: $bar.length, wrapper: $wrapper.length}); } catch(e) {}
			return;
		}
		...
		// Hide bar and next button on introPage DOM presence
		if ($("#introPage").length) {
			$wrapper.hide();
			$nextTop.hide();
			$bar.css("width", "0%");
		...
		// Level-based behaviour in auto-generate mode
		if (typeof x_autoGenerateMode !== "undefined" &&
			x_autoGenerateMode &&
			typeof x_virtualPages !== "undefined" &&
			x_virtualPages &&
			typeof x_pageInfo !== "undefined" &&
			typeof x_currentPage !== "undefined") {

			// Map current page to a virtual index
			var baseIndex = x_pageInfo.length - x_virtualPages.length;
			var currentVirtualIndex = x_currentPage - baseIndex;

			if (currentVirtualIndex >= 0 && currentVirtualIndex < x_virtualPages.length) {
				var currentVirtualPage = x_virtualPages[currentVirtualIndex];
				if (currentVirtualPage && typeof currentVirtualPage.level === "number") {
					var level = currentVirtualPage.level;

					// Collect all exercise virtual pages for this level
					var levelVirtualIndices = [];
					for (var vi = 0; vi < x_virtualPages.length; vi++) {
						var vp = x_virtualPages[vi];
						if (!vp) { continue; }
						if (vp.level !== level) { continue; }
						if (vp.type === "levelCompletion" || vp.type === "colofon") { continue; }
						levelVirtualIndices.push(vi);
					}

					if (levelVirtualIndices.length > 0) {
						var posInLevel = levelVirtualIndices.indexOf(currentVirtualIndex);
						if (posInLevel >= 0) {
							// Simple positional progress: first exercise = 1/N, last = 100%
							percentage = ((posInLevel + 1) / levelVirtualIndices.length) * 100;
						} else {
							// On non-exercise virtual page within this level (e.g. completion) -> 0%
							percentage = 0;
						}
					}
				}
			} else {
				// Not on a virtual page (intro/title/woordenlijst) -> 0% for top bar
				percentage = 0;
			}
		} else if (typeof x_normalPages !== "undefined" && typeof x_currentPage !== "undefined") {
			// Fallback: overall progress over all normal pages
			var totalPages = x_normalPages.length || 1;
			var currentIndex = x_normalPages.indexOf(x_currentPage);
			if (currentIndex < 0) { currentIndex = 0; }
			percentage = ((currentIndex + 1) / totalPages) * 100;
		}

		// Apply width
		$bar.css("width", (percentage || 0) + "%");

		// Also update numeric progress if element exists
		try {
			var $count = $("#x_progressCount");
			if ($count.length) {
				var currentCount = 0;
				var totalCount = 0;

				if (typeof x_autoGenerateMode !== "undefined" && x_autoGenerateMode && typeof x_virtualPages !== "undefined" && x_virtualPages && typeof x_pageInfo !== "undefined" && typeof x_currentPage !== "undefined") {
					// compute counts for current level
					var baseIndex = x_pageInfo.length - x_virtualPages.length;
					var currentVirtualIndex = x_currentPage - baseIndex;
					if (currentVirtualIndex >= 0 && currentVirtualIndex < x_virtualPages.length) {
						var currentVirtualPage = x_virtualPages[currentVirtualIndex];
						if (currentVirtualPage && typeof currentVirtualPage.level === "number") {
							...
							totalCount = levelVirtualIndices.length;
							var posInLevel = levelVirtualIndices.indexOf(currentVirtualIndex);
							currentCount = posInLevel >= 0 ? posInLevel + 1 : 0;
						}
					}
				} else if (typeof x_normalPages !== "undefined" && typeof x_currentPage !== "undefined") {
					totalCount = x_normalPages.length || 0;
					var currentIndex = x_normalPages.indexOf(x_currentPage);
					currentCount = currentIndex >= 0 ? currentIndex + 1 : 0;
				}

				if (totalCount > 0 && currentCount >= 0) {
					$count.text(currentCount + '/' + totalCount);
					try { console.log('x_progressCount -> ' + currentCount + '/' + totalCount + ' (pct: ' + Math.round(percentage) + '%)'); } catch(e) {}
				} else {
					$count.html('&nbsp;');
				}
			}
		...
		// Show wrapper/next button on non-intro pages
		if (!$("#introPage").length) {
			$wrapper.show();
			if ($nextTop.length) {
				$nextTop.show();
...
var XENITH = (function ($, parent) {
	...
	function build($parent) {
		...
		if (XENITH.PAGEMENU.menuPage === true) {
			...
			// lightbox
			if ($('#menuHolder').parents('.featherlight').length > 0) {
				$('#menuHolder').prepend('<h1 id="x_introH1">' + (x_params.tocLabel != undefined && x_params.tocLabel != "" ? x_params.tocLabel : x_getLangInfo(x_languageData.find("toc")[0], "label", "Inhoudsopgave")) + '</h1>');
	...
	function buildDialog() {
		x_openDialog(
			"menu",
			x_params.tocLabel != undefined && x_params.tocLabel != "" ? x_params.tocLabel : x_getLangInfo(x_languageData.find("toc")[0], "label", "Inhoudsopgave"),
			x_getLangInfo(x_languageData.find("toc").find("closeButton")[0], "description", "Sluit Inhoudsopgave"),
			null,
...
var XENITH = (function ($, parent) {
	...
	function update(page, target) {
		if (typeof console !== 'undefined' && console.log) {
			console.log('PROGRESSBAR.update() CALLED - page:', page, 'target:', target, 'progressBar:', progressBar);
		}
		if (progressBar) {
			let update = true;
			...
			if (typeof console !== 'undefined' && console.log) {
				console.log('PROGRESSBAR.update() - update flag:', update, 'x_autoGenerateMode:', typeof x_autoGenerateMode !== 'undefined' ? x_autoGenerateMode : 'undefined', 'x_currentPage:', typeof x_currentPage !== 'undefined' ? x_currentPage : 'undefined');
			}
			
			if (update === true) {
				// progress bar needs to be updated
				
				// Check if we're in auto-generate mode and should limit progress to current level
				var currentLevelStartIndex = null;
				var currentLevelEndIndex = null;
				var currentLevel = null;
				if (x_autoGenerateMode && typeof x_currentPage !== 'undefined') {
					currentLevel = x_getCurrentLevel(x_currentPage);
					
					// If we're on a levelCompletion page, get the level from the page itself
					if (currentLevel === null && x_pageInfo[x_currentPage] && x_pageInfo[x_currentPage].type === 'levelCompletion') {
						// Try to get level from virtual page data
						var baseIndex = x_pageInfo.length - (typeof x_virtualPages !== 'undefined' && x_virtualPages ? x_virtualPages.length : 0);
						if (x_currentPage >= baseIndex) {
							var virtualIndex = x_currentPage - baseIndex;
							if (typeof x_virtualPages !== 'undefined' && x_virtualPages && virtualIndex >= 0 && virtualIndex < x_virtualPages.length) {
								var vp = x_virtualPages[virtualIndex];
								if (vp && vp.config && typeof vp.config.level === 'number') {
									currentLevel = vp.config.level;
								}
							}
						}
					}
					...
					if (currentLevel !== null) {
						currentLevelStartIndex = x_getLevelStartIndex(currentLevel);
						currentLevelEndIndex = x_getLevelEndIndex(currentLevel);
					}
				}
				...
				if (progressSub != false) {
					...
					pagesViewed = $(x_pageInfo).filter(function (i) {
						// If we're in a level, only count pages within that level range
						if (currentLevelStartIndex !== null && i < currentLevelStartIndex) {
							return false; // Before level start
						}
						if (currentLevelEndIndex !== null && i > currentLevelEndIndex) {
							return false; // After level end
						}
						// Exclude standalone pages, menu pages, and levelCompletion pages
						if (this.standalone === true || this.type === "menu" || this.type === "levelCompletion") {
							return false;
						}
				...
				// Calculate total pages for progress calculation
				// If we're in a level, only count exercise pages within that level range
				var totalPagesForProgress = totalPages;
				if (currentLevelStartIndex !== null && currentLevelEndIndex !== null) {
					// Count total pages in the range (inclusive), excluding completion pages
					// This gives us the total number of exercise pages in this level for progress calculation
					var pagesInRange = $(x_pageInfo).filter(function (i) {
						if (i < currentLevelStartIndex) {
							return false;
						}
						if (i > currentLevelEndIndex) {
							return false;
						}
						// Exclude standalone pages, menu pages, and levelCompletion pages
						if (this.standalone === true || this.type === "menu" || this.type === "levelCompletion") {
					...
					totalPagesForProgress = pagesInRange.length;
				} else if (currentLevelEndIndex !== null) {
					// Fallback: if we only have end index, count from page 0
					totalPagesForProgress = currentLevelEndIndex + 1; // +1 because indices are 0-based
				}
				
				pagesViewed = pagesViewed.length;
				const progress = totalPagesForProgress > 0 ? Math.round((pagesViewed * 100) / totalPagesForProgress) : 0;
				
				// Debug logging - always log if in auto-generate mode
				if (typeof console !== 'undefined' && console.log) {
					if (x_autoGenerateMode) {
						console.log('=== PROGRESS DEBUG ===');
						console.log('x_autoGenerateMode:', x_autoGenerateMode);
						console.log('Current Page:', x_currentPage);
						console.log('Current Page Type:', x_pageInfo[x_currentPage] ? x_pageInfo[x_currentPage].type : 'unknown');
						console.log('Current Level:', currentLevel);
						console.log('Level Start Index:', currentLevelStartIndex);
						console.log('Level End Index:', currentLevelEndIndex);
						console.log('Pages Viewed:', pagesViewed, '| Total Pages:', totalPagesForProgress);
						console.log('Progress:', progress + '%');
						
						// Show what pages are in range
						if (currentLevelStartIndex !== null && currentLevelEndIndex !== null) {
							console.log('Pages in range:');
							for (var i = currentLevelStartIndex; i <= currentLevelEndIndex && i < x_pageInfo.length; i++) {
								console.log('  Page', i, '- Type:', x_pageInfo[i].type, 'Viewed:', x_pageInfo[i].viewed !== false);
							}
						}
						console.log('====================');
					}
				...
				if (progressSub == false) {
					...
					if (progressSub == "pages") {
						...
						$(x_pageInfo).each(function (i) {
							// Apply level limits if in auto-generate mode
							if (currentLevelStartIndex !== null && i < currentLevelStartIndex) {
								return; // Skip pages before level start
							}
							if (currentLevelEndIndex !== null && i > currentLevelEndIndex) {
								return; // Skip pages after level end
							}
	...
	self.init = init;
	self.build = build;
	self.update = function(page, target) {
		if (typeof console !== 'undefined' && console.log) {
			console.log('PUBLIC update() called with page:', page, 'target:', target);
		}
		update(page, target);
	};
...
// _____ FEEDBACK NOTIFICATION _____
// Shared success feedback notification for exercises
var XENITH = (function ($, parent) {
	var self = parent.FEEDBACK = {};

	// Show professional success notification
	self.showSuccess = function(options) {
		console.log('showing success notification with options:', options);
		options = options || {};
		var container = options.container || document.getElementById('x_pageDiv') || document.body;
		var message = options.message || 'goed gedaan!';
		var showConfetti = options.showConfetti !== false; // default true
		var playSound = options.playSound !== false; // default true
		var duration = options.duration || 3000; // default 3 seconds

		// Remove any existing notification
		var existing = container.querySelector('.wt-success-notification');
		if (existing) {
			existing.remove();
		}
		...
		notification.className = 'wt-success-notification';
		notification.innerHTML = '<div class="wt-success-content"><i class="fa fa-check-circle"></i><span class="wt-success-text">' + message + '</span></div>';
		
		// Add styles if not already added
		if (!document.getElementById('wt-success-notification-styles')) {
			var style = document.createElement('style');
			style.id = 'wt-success-notification-styles';
			style.textContent = `
				.wt-success-notification {
					position: fixed;
					bottom: -0px;
					left: 50%;
		...
		// Ensure container has relative positioning
		if (container && container.style.position !== 'relative' && container.style.position !== 'absolute' && container.style.position !== 'fixed') {
...
// Detect if auto-generation should be activated
// Only activates if project contains ONLY: introPage, woordenLijstExtended, and possibly title/intro pages
// If other pages exist (connectorMenu, exercises, etc.), skip auto-generation
function x_detectAutoGenerate() {
	x_autoGenerateMode = false;
	x_virtualPages = [];
	x_wordListData = [];

	// Count page types and find specific pages
	var pageTypes = {};
	var hasWordList = false;
	var hasExercises = false;
	var hasConnector = false;
	var wordListPageIndex = -1;
	var hasIntroPage = false;
	var hasTitlePage = false;
	var introPageIndex = -1;
	var titlePageIndex = -1;
	...
	for (var i = 0; i < x_pageInfo.length; i++) {
		...
		// Check for intro page - multiple variations possible
		// Check both pageType and tagName
		var isIntroType = pageType === 'intropage' || pageType === 'intro' || pageType === 'introduction';
		var isIntroTag = tagName === 'intropage' || tagName === 'intro' || tagName === 'introduction';
		...
		// Check for exercise pages (indicates manual setup)
		var exerciseTypes = ['flashcards', 'meerkeuzevraag', 'woordmemory', 'herhaalDeZin', 'woordverdwijner', 
		                     'woordtrainerfillblanks', 'woordtrainerquiz', 'woordzoeker', 'woordtrainerquizspelling'];
		if (exerciseTypes.indexOf(pageType) !== -1) {
			hasExercises = true;
		}

		// Check for connector/menu pages (indicates manual setup)
		if (pageType === 'connectormenu' || pageType === 'navigator') {
			hasConnector = true;
		}
	}

	// If we have a word list but no intro page detected, check if there's a page before the word list
	// This could be an intro page with a different type name (but NOT 'title' - title is for level selection)
	if (hasWordList && wordListPageIndex > 0 && introPageIndex === -1) {
		// Check if there's a page before the word list that might be an intro page
		for (var i = 0; i < wordListPageIndex; i++) {
			var pageXML = x_pages[i];
			var pageType = x_pageInfo[i].type ? x_pageInfo[i].type.toLowerCase() : '';
			var tagName = '';
			if (pageXML && typeof pageXML.tagName !== 'undefined') {
			...
			// Don't treat 'title' as intro - title is for level selection
			// Only treat actual intro page types as intro
			if (pageType === 'intropage' || pageType === 'intro' || pageType === 'introduction' ||
			    tagName === 'intropage' || tagName === 'intro' || tagName === 'introduction') {
				// Actual intro page type
				introPageIndex = i;
				hasIntroPage = true;
				break;
			} else if (pageType !== 'title' && pageType !== 'woordenlijstextended' && pageType !== 'woordenlijstcustom' &&
			           tagName !== 'title' && tagName !== 'woordenlijstextended' && tagName !== 'woordenlijstcustom') {
				// Any other page (except title and word list) before word list = potential intro page
				introPageIndex = i;
				hasIntroPage = true;
				break;
			}
	...
	// 1. Has word list
	// 2. No existing exercises
	// 3. No connector menus
	// 4. Max 3 pages (introPage/intro, woordenLijstExtended, title for level selection)
	// Note: introPage/intro and title are different - introPage is the introduction, title is for level selection
	if (hasWordList && !hasExercises && !hasConnector && x_pageInfo.length <= 3) {
		// Parse word list data
		x_wordListData = x_getWordListData(wordListPageIndex);
		
	// Need at least 1 word to generate exercises
	if (x_wordListData.length > 0) {
		x_autoGenerateMode = true;
		
		// Generate exercises first (this will detect if virtual introPage is needed)
		x_generateExercises();
		
		// Ensure correct order: introPage (virtual or real) -> title (level selection) -> woordenlijst -> exercises
		if (introPageIndex >= 0 || wordListPageIndex >= 0 || titlePageIndex >= 0 || (typeof x_virtualIntroPage !== 'undefined' && x_virtualIntroPage)) {
			// Store if custom start page was set via URL parameters
			var hadCustomStartPage = (x_urlParams.linkID || x_urlParams.pageID || x_urlParams.page || x_urlParams.resume || (window.location.hash && window.location.hash !== '#pageContents'));
			
			x_ensureCorrectOrder(introPageIndex, wordListPageIndex, titlePageIndex);
			
			// After reordering, set start page.
			// We want to begin on the woordenlijst page (if present), otherwise fall back to index 0.
			if (!hadCustomStartPage && x_pageInfo.length > 0) {
				var startIndex = 0;
				for (var si = 0; si < x_pageInfo.length; si++) {
					var sType = x_pageInfo[si].type ? x_pageInfo[si].type.toLowerCase() : '';
					if (sType === 'woordenlijstextended' || sType === 'woordenlijst extended' ||
					    sType === 'woordenlijstcustom'   || sType === 'woordenlijst custom') {
						startIndex = si;
						break;
					}
				}
				x_startPage = { type: "index", ID: String(startIndex) };
			}
		}
	}
}
...
// Ensure pages are in correct order: introPage (virtual or real) -> title -> woordenlijst -> exercises
function x_ensureCorrectOrder(introPageIndex, wordListPageIndex, titlePageIndex) {
	// Reorder x_pageInfo and x_normalPages to ensure correct sequence
	// Expected order: 
	//   - introPage (0) - virtual if no real one exists
	//   - title (1) - level selection
	//   - woordenlijst (2)
	//   - exercises (3+)
	var correctOrder = [];
	var seenIndices = {};

	// Check if we need to add a virtual introPage
	var needsVirtualIntro = (introPageIndex === -1 && typeof x_virtualIntroPage !== 'undefined' && x_virtualIntroPage);

	// Store original page data for reordering
	var pageData = [];
	for (var i = 0; i < x_pageInfo.length; i++) {
		pageData.push({
			pageInfo: x_pageInfo[i],
			pageXML: x_pages[i]
		});
	}

	// If we need a virtual introPage, create it and add it first
	if (needsVirtualIntro) {
		// Create virtual introPage entry
		var virtualIntroPageInfo = {
			type: 'introPage',
			built: false,
			viewed: false,
			linkID: 'VIRTUAL_INTRO',
			virtual: true,
			childIDs: [],
			isVirtualIntro: true
		};
		var virtualIntroPageXML = document.createElement('introPage');
		virtualIntroPageXML.setAttribute('name', 'introPage');
		virtualIntroPageXML.setAttribute('virtual', 'true');
		
		// Insert at the beginning of arrays
		pageData.unshift({
			pageInfo: virtualIntroPageInfo,
			pageXML: virtualIntroPageXML
		});
		
		// Adjust indices for other pages
		if (titlePageIndex >= 0) titlePageIndex++;
		if (wordListPageIndex >= 0) wordListPageIndex++;
		introPageIndex = 0; // Virtual introPage is now at index 0
	}

	// Add introPage first (virtual or real)
	if (introPageIndex >= 0 && introPageIndex < pageData.length) {
		correctOrder.push(introPageIndex);
		seenIndices[introPageIndex] = true;
	}

	// Add title second (level selection)
	if (titlePageIndex >= 0 && titlePageIndex < pageData.length) {
		correctOrder.push(titlePageIndex);
		seenIndices[titlePageIndex] = true;
	}

	// Add woordenlijst third
	if (wordListPageIndex >= 0 && wordListPageIndex < pageData.length) {
		correctOrder.push(wordListPageIndex);
		seenIndices[wordListPageIndex] = true;
	}
	...
	// Reorder x_pageInfo and x_pages arrays
	var reorderedPageInfo = [];
	var reorderedPages = [];
	for (var i = 0; i < correctOrder.length; i++) {
		var origIndex = correctOrder[i];
		reorderedPageInfo.push(pageData[origIndex].pageInfo);
		reorderedPages.push(pageData[origIndex].pageXML);
	}

	// Update arrays (need to preserve jQuery object for x_pages)
	x_pageInfo = reorderedPageInfo;
	
	// Recreate x_pages jQuery object from reordered pages
	var tempPages = [];
	for (var i = 0; i < reorderedPages.length; i++) {
		tempPages.push(reorderedPages[i]);
	}
	x_pages = $(tempPages);

	// Update x_normalPages indices
	x_normalPages = [];
	for (var i = 0; i < x_pageInfo.length; i++) {
		if (x_pageInfo[i].standalone != true) {
			x_normalPages.push(i);
...
// Get word list data from woordenLijstExtended or woordenLijstCustom page
function x_getWordListData(pageIndex) {
	var words = [];

	try {
		if (pageIndex < 0 || pageIndex >= x_pages.length) {
			return words;
		}

		var pageXML = x_pages[pageIndex];
		if (!pageXML || typeof pageXML.getAttribute !== 'function') {
			return words;
		}

		// Check if this is a woordenlijstcustom page
		var pageType = x_pageInfo[pageIndex] ? x_pageInfo[pageIndex].type : '';
		var tagName = '';
		if (pageXML && typeof pageXML.tagName !== 'undefined') {
			tagName = pageXML.tagName.toLowerCase();
		}
		var isCustomPage = pageType === 'woordenlijstcustom' || pageType === 'woordenlijst custom' || 
		                   tagName === 'woordenlijstcustom';

		// For woordenlijstcustom pages, ONLY use localStorage (ignore XML wordListText)
		if (isCustomPage) {
			try {
				var saved = localStorage.getItem('wt:customWordSet');
				if (saved) {
					var customWords = JSON.parse(saved);
					if (Array.isArray(customWords) && customWords.length > 0) {
						// Convert custom word set format to the format expected by auto-generation
						for (var i = 0; i < customWords.length; i++) {
							var w = customWords[i];
							words.push({
								word: w.word || '',
								betekenis: w.betekenis || '',
								contextzinExpliciet: w.contextzinExpliciet || '',
								contextzinImpliciet: w.contextzinImpliciet || ''
							});
						}
						return words; // Return early - only use localStorage for custom pages
					}
				}
			} catch(e) {
				console.error('Error reading custom word set from localStorage:', e);
			}
			// If no words in localStorage, return empty array (don't fall through to XML parsing)
			return words;
		}

		// For regular woordenlijstExtended pages, use XML wordListText
		var wordListText = pageXML.getAttribute('wordListText') || '';
		if (!wordListText || wordListText.trim() === '') {
			return words;
		}

		// Parse the text format (same as woordenLijstExtended.html)
		var wordBlocks = wordListText.split(/\n\s*\n/);

		// If no double newlines, try to detect word blocks by pattern
		if (wordBlocks.length === 1) {
			var allLines = wordListText.split('\n').map(function(l) { return l.trim(); }).filter(function(l) { return l.length > 0; });
			wordBlocks = [];
			var currentBlock = [];

			for (var i = 0; i < allLines.length; i++) {
				var line = allLines[i];
				var wordMatch = line.match(/^([^:]+):(.+)$/);
				var isNewWord = wordMatch && !line.toLowerCase().startsWith('contextzin');
		...
		// Parse each word block
		for (var i = 0; i < wordBlocks.length; i++) {
			var block = wordBlocks[i];
			var lines = block.split('\n').map(function(l) { return l.trim(); }).filter(function(l) { return l.length > 0; });
			if (lines.length === 0) continue;

			var word = '';
			var betekenis = '';
			var contextzinImpliciet = '';
			var contextzinExpliciet = '';

			for (var j = 0; j < lines.length; j++) {
				var line = lines[j];
				var wordMatch = line.match(/^([^:]+):(.+)$/);
				if (wordMatch && !line.toLowerCase().startsWith('contextzin')) {
					word = x_stripHtml(wordMatch[1].trim());
					betekenis = x_stripHtml(wordMatch[2].trim());
				} else if (line.toLowerCase().startsWith('contextzin (i):')) {
					contextzinImpliciet = x_stripHtml(line.substring(15).trim());
				} else if (line.toLowerCase().startsWith('contextzin (e):')) {
					contextzinExpliciet = x_stripHtml(line.substring(15).trim());
				}
			}
			...
			if (word && betekenis) {
				words.push({
					word: word,
					betekenis: betekenis,
					contextzinExpliciet: contextzinExpliciet,
					contextzinImpliciet: contextzinImpliciet
				});
			}
		}
	} catch (e) {
...
// Generate exercises for all levels
function x_generateExercises() {
	if (!x_autoGenerateMode || x_wordListData.length === 0) {
		return;
	}

	// Check if introPage exists, if not create a virtual one
	// We need to check this BEFORE adding exercises
	var hasRealIntroPage = false;
	for (var i = 0; i < x_pageInfo.length; i++) {
		var pageType = x_pageInfo[i].type ? x_pageInfo[i].type.toLowerCase() : '';
		var pageXML = x_pages[i];
		var tagName = '';
		...
		if (pageType === 'intropage' || tagName === 'intropage' || tagName === 'intro') {
			hasRealIntroPage = true;
			break;
		}
	}
	
	// If no introPage exists, create a virtual one at the beginning
	if (!hasRealIntroPage) {
		// Create virtual introPage that will be inserted at index 0
		// We'll store it in a special array and insert it during reordering
		x_virtualIntroPage = {
			type: 'introPage',
			virtual: true,
			isVirtualIntro: true
		...
		x_virtualIntroPage = null;
	}

	var virtualPageIndex = x_pageInfo.length;
	var baseVirtualIndex = virtualPageIndex;

	// Level 1 exercises (expliciete data)
	var level1Exercises = ['flashCards', 'meerkeuzevraag', 'woordMemory', 'herhaalDeZin', 'woordverdwijner', 'woordShuffle'];
	var level1Count = x_wordListData.length * 2; // 2 exercises per word

	// Level 2 exercises (impliciete data)
	var level2Exercises = ['woordtrainerFillBlanks', 'woordtrainerFillBlanks', 'woordtrainerQuiz', 'associaties', 'woordMemory'];
	var level2Count = x_wordListData.length * 2;

	// Level 3 exercises (spelling + alles)
	// Note: woordtrainerFillBlanks in level 3 uses typing mode
	var level3Exercises = ['woordtrainerFillBlanks', 'woordzoeker', 'herhaalDeZin', 'woordtrainerQuizSpelling'];
	var level3Count = x_wordListData.length * 2;

	// Practice-based weights: less-practiced words get higher weight so they appear more often
	var wordWeights = x_getWordPracticeWeights(x_wordListData);
	// Create exercise assignments for each level (with weights so under-practiced words appear more)
	var level1Assignments = x_createExerciseAssignments(x_wordListData, level1Exercises, level1Count, wordWeights);
	var level2Assignments = x_createExerciseAssignments(x_wordListData, level2Exercises, level2Count, wordWeights);
	var level3Assignments = x_createExerciseAssignments(x_wordListData, level3Exercises, level3Count, wordWeights);

	// Ensure exactly 20 exercises per level by generating additional assignments if needed
	var TARGET_EXERCISES_PER_LEVEL = 20;
	
	// Helper function to generate virtual pages and ensure exactly TARGET_EXERCISES_PER_LEVEL
	function generateLevelVirtualPages(assignments, level, targetCount) {
		var generatedPages = [];
		var attempts = 0;
		var maxAttempts = assignments.length * 3; // Prevent infinite loops
		
		// First pass: generate pages from existing assignments
		for (var i = 0; i < assignments.length && generatedPages.length < targetCount; i++) {
			var assignment = assignments[i];
			var virtualPageData = {
				index: virtualPageIndex++,
				level: level,
				type: assignment.exerciseType,
				wordData: assignment.wordData,
				exerciseIndex: generatedPages.length,
				totalExercises: targetCount
			};
			virtualPageData.config = x_getExerciseConfig(assignment.exerciseType, assignment.wordData, virtualPageData);
			if (virtualPageData.config !== null) {
				generatedPages.push(virtualPageData);
			} else {
				virtualPageIndex--; // Decrement since we're not adding this page
			}
		}
		
		// If we don't have enough, generate more from available words
		while (generatedPages.length < targetCount && attempts < maxAttempts) {
			attempts++;
			// Pick a random exercise type from the level's exercise types
			var exerciseTypes = level === 1 ? level1Exercises : (level === 2 ? level2Exercises : level3Exercises);
			var randomExerciseType = exerciseTypes[Math.floor(Math.random() * exerciseTypes.length)];
			// Pick a word: prefer less-practiced (wordWeights from outer scope)
			var allIndices = [];
			for (var k = 0; k < x_wordListData.length; k++) allIndices.push(k);
			var wi = (typeof wordWeights !== 'undefined' && wordWeights && wordWeights.length === x_wordListData.length)
				? x_pickWeightedWordIndex(allIndices, wordWeights) : Math.floor(Math.random() * x_wordListData.length);
			var randomWordData = x_wordListData[wi];
			...
			virtualPageData.config = x_getExerciseConfig(randomExerciseType, randomWordData, virtualPageData);
			if (virtualPageData.config !== null) {
				generatedPages.push(virtualPageData);
			} else {
				virtualPageIndex--; // Decrement since we're not adding this page
		...
		// Trim to exactly targetCount if we have more
		if (generatedPages.length > targetCount) {
			var excess = generatedPages.length - targetCount;
			generatedPages = generatedPages.slice(0, targetCount);
			// Adjust virtualPageIndex accordingly
			virtualPageIndex -= excess;
		}
		
		return generatedPages;
	}

	// Store level start indices for completion tracking
	var level1StartIndex = virtualPageIndex;
	var level2StartIndex;
	var level3StartIndex;

	// Generate virtual pages for Level 1 (exactly 20 exercises)
	var level1Pages = generateLevelVirtualPages(level1Assignments, 1, TARGET_EXERCISES_PER_LEVEL);
	for (var i = 0; i < level1Pages.length; i++) {
		x_virtualPages.push(level1Pages[i]);
	}

	// Add Level 1 completion page
	level2StartIndex = virtualPageIndex;
	var level1EndIndex = virtualPageIndex - 1; // Last exercise page before completion page
	x_virtualPages.push({
		index: virtualPageIndex++,
		level: 1,
		...
		config: {
			level: 1,
			levelStartIndex: level1StartIndex,
			levelEndIndex: level1EndIndex,
			totalExercises: TARGET_EXERCISES_PER_LEVEL, // Pass exact count
			nextLevelStartIndex: level2StartIndex
		}
	});

	// Generate virtual pages for Level 2 (exactly 20 exercises)
	var level2Pages = generateLevelVirtualPages(level2Assignments, 2, TARGET_EXERCISES_PER_LEVEL);
	for (var i = 0; i < level2Pages.length; i++) {
		x_virtualPages.push(level2Pages[i]);
	}

	// Add Level 2 completion page
	level3StartIndex = virtualPageIndex;
	var level2EndIndex = virtualPageIndex - 1; // Last exercise page before completion page
	x_virtualPages.push({
		index: virtualPageIndex++,
		level: 2,
		type: 'levelCompletion',
		...
		config: {
			level: 2,
			levelStartIndex: level2StartIndex,
			levelEndIndex: level2EndIndex,
			totalExercises: TARGET_EXERCISES_PER_LEVEL, // Pass exact count
			nextLevelStartIndex: level3StartIndex
		}
	});

	// Generate virtual pages for Level 3 (exactly 20 exercises)
	var level3Pages = generateLevelVirtualPages(level3Assignments, 3, TARGET_EXERCISES_PER_LEVEL);
	for (var i = 0; i < level3Pages.length; i++) {
		x_virtualPages.push(level3Pages[i]);
	}

	// Add Level 3 completion page
	var level3EndIndex = virtualPageIndex - 1; // Last exercise page before completion page
	x_virtualPages.push({
		index: virtualPageIndex++,
		level: 3,
		type: 'levelCompletion',
		...
		config: {
			level: 3,
			levelStartIndex: level3StartIndex,
			levelEndIndex: level3EndIndex,
			totalExercises: TARGET_EXERCISES_PER_LEVEL, // Pass exact count
			nextLevelStartIndex: -1 // No next level
		}
	});

	// Add colofon at very end
	x_virtualPages.push({
		index: virtualPageIndex++,
		level: 3,
		type: 'colofon',
	...
	// Update x_normalPages to include virtual pages
	for (var i = 0; i < x_virtualPages.length; i++) {
		x_normalPages.push(baseVirtualIndex + i);
		// Map flashCards to newFlashCard for proper styling
		// Map woordtrainerQuizSpelling to woordspellingAudio for correct HTML file
		var pageType = x_virtualPages[i].type;
		if (pageType === 'flashCards') {
			pageType = 'newFlashCard';
		} else if (pageType === 'woordtrainerQuizSpelling') {
			pageType = 'woordspellingAudio';
		}
		x_pageInfo.push({
			type: pageType,
			built: false,
			...
			linkID: 'VIRTUAL_' + i,
			virtual: true,
			childIDs: []  // Required property to prevent errors in x_checkPages
		});
	}
}

// Reset localStorage progress for one level (wt:stats, wt:done) by page number so old keys are removed too.
function x_resetLevelProgressInStorage(level) {
	try {
		if (typeof x_virtualPages === 'undefined' || !x_virtualPages || x_virtualPages.length === 0) return;
		var TARGET = 20;
		var exerciseStart = (level - 1) * 21;
		var exerciseEnd = exerciseStart + TARGET - 1;
		if (exerciseEnd >= x_virtualPages.length) return;
		var baseIndex = x_pageInfo.length - x_virtualPages.length;
		var pageNums = {};
		for (var i = exerciseStart; i <= exerciseEnd; i++) {
			pageNums[baseIndex + i + 1] = true; // 1-based actual page numbers
		}
		var prefix = 'wt:stats:' + (location.pathname || '') + (location.search || '');
		var donePrefix = 'wt:done:' + (location.pathname || '') + (location.search || '');
		var statsData = {};
		var doneData = {};
		try { statsData = JSON.parse(localStorage.getItem(prefix) || '{}'); } catch (e) {}
		try { doneData = JSON.parse(localStorage.getItem(donePrefix) || '{}'); } catch (e) {}
		function removeKeysForLevel(obj) {
			var keysToRemove = [];
			for (var key in obj) {
				if (!obj.hasOwnProperty(key) || key.indexOf('@p') === -1) continue;
				var num = key.split('@p')[1];
				if (num && pageNums[parseInt(num, 10)]) keysToRemove.push(key);
			}
			for (var k = 0; k < keysToRemove.length; k++) delete obj[keysToRemove[k]];
		}
		removeKeysForLevel(statsData);
		removeKeysForLevel(doneData);
		localStorage.setItem(prefix, JSON.stringify(statsData));
		localStorage.setItem(donePrefix, JSON.stringify(doneData));
		try { window.dispatchEvent(new CustomEvent('wt:statsUpdated', { detail: { key: prefix } })); } catch (e) {}
	} catch (e) {}
}

// Regenerate virtual exercise pages for one level (so replay gives different exercises).
// Call when user clicks "level X opnieuw oefenen" or when choosing a level from title. Replaces only that level's exercise slice.
function x_regenerateLevelExercises(level) {
	if (!x_autoGenerateMode || typeof x_virtualPages === 'undefined' || !x_virtualPages || x_virtualPages.length === 0) {
		return;
	}
	...
	var TARGET = 20;
	// Virtual page indices: level 1 = 0..19, level 2 = 21..40, level 3 = 42..61 (20 exercises each, completion in between)
	var exerciseStart = (level - 1) * 21;
	var exerciseEnd = exerciseStart + TARGET - 1;
	if (exerciseEnd >= x_virtualPages.length) {
		return;
	}
	// Clear localStorage progress for this level first (by page number so old exercise-type keys are removed)
	x_resetLevelProgressInStorage(level);
	var newPages = x_generateLevelVirtualPagesForReplay(level, exerciseStart, TARGET);
	if (!newPages || newPages.length !== TARGET) {
		return;
	}
	// Replace slice in x_virtualPages
	for (var i = 0; i < TARGET; i++) {
		x_virtualPages[exerciseStart + i] = newPages[i];
	}
	var baseIndex = x_pageInfo.length - x_virtualPages.length;
	for (var j = 0; j < TARGET; j++) {
		var pageType = newPages[j].type;
		if (pageType === 'flashCards') {
			pageType = 'newFlashCard';
		} else if (pageType === 'woordtrainerQuizSpelling') {
			pageType = 'woordspellingAudio';
		}
		x_pageInfo[baseIndex + exerciseStart + j] = {
			type: pageType,
			built: false,
			viewed: false,
			linkID: 'VIRTUAL_' + (exerciseStart + j),
			virtual: true,
			childIDs: []
		};
		// Clear cached page XML so it is rebuilt from new x_virtualPages
		var pageIdx = baseIndex + exerciseStart + j;
		if (typeof x_pages !== 'undefined' && pageIdx >= 0 && pageIdx < x_pages.length) {
			x_pages[pageIdx] = null;
		}
	}
}

// Generate virtual page data for one level (for replay). Returns array of virtual page objects.
// Uses practice weights so less-practiced words appear more often.
function x_generateLevelVirtualPagesForReplay(level, startVirtualIndex, targetCount) {
	if (typeof x_wordListData === 'undefined' || !x_wordListData || x_wordListData.length === 0) {
		return [];
	}
	var level1Exercises = ['flashCards', 'meerkeuzevraag', 'woordMemory', 'herhaalDeZin', 'woordverdwijner', 'woordShuffle'];
	var level2Exercises = ['woordtrainerFillBlanks', 'woordtrainerFillBlanks', 'woordtrainerQuiz', 'associaties', 'woordMemory'];
	var level3Exercises = ['woordtrainerFillBlanks', 'woordzoeker', 'herhaalDeZin', 'woordtrainerQuizSpelling'];
	var level1Count = x_wordListData.length * 2;
	var level2Count = x_wordListData.length * 2;
	var level3Count = x_wordListData.length * 2;
	var exerciseTypes = level === 1 ? level1Exercises : (level === 2 ? level2Exercises : level3Exercises);
	var count = level === 1 ? level1Count : (level === 2 ? level2Count : level3Count);
	var wordWeights = x_getWordPracticeWeights(x_wordListData);
	var assignments = x_createExerciseAssignments(x_wordListData, exerciseTypes, count, wordWeights);
	var generatedPages = [];
	var attempts = 0;
	var maxAttempts = assignments.length * 3;
	for (var i = 0; i < assignments.length && generatedPages.length < targetCount; i++) {
		var assignment = assignments[i];
		var virtualPageData = {
			index: startVirtualIndex + generatedPages.length,
			level: level,
			type: assignment.exerciseType,
			wordData: assignment.wordData,
			exerciseIndex: generatedPages.length,
	...
	while (generatedPages.length < targetCount && attempts < maxAttempts) {
		attempts++;
		var randomExerciseType = exerciseTypes[Math.floor(Math.random() * exerciseTypes.length)];
		// Prefer less-practiced words (weighted pick)
		var allIndices = [];
		for (var k = 0; k < x_wordListData.length; k++) allIndices.push(k);
		var wi = (wordWeights && wordWeights.length === x_wordListData.length)
			? x_pickWeightedWordIndex(allIndices, wordWeights) : Math.floor(Math.random() * x_wordListData.length);
		var randomWordData = x_wordListData[wi];
		var virtualPageData = {
			index: startVirtualIndex + generatedPages.length,
...
// Return practice weights for words: higher weight = less practiced and/or more often wrong.
// Uses wt:globalWordStats (keys = lowercase word, value = { right, wrong }).
// 1) Less practiced => higher weight. 2) Higher error rate (wrong/(right+wrong)) => extra boost so words that are often wrong appear more.
function x_getWordPracticeWeights(words) {
	if (!words || words.length === 0) return [];
	var weights = [];
	var globalStats = {};
	try {
		globalStats = JSON.parse(localStorage.getItem('wt:globalWordStats') || '{}');
	} catch (e) {}
	for (var i = 0; i < words.length; i++) {
		var w = words[i];
		var wordStr = (w && w.word != null) ? String(w.word).trim() : '';
		var key = wordStr.toLowerCase();
		var stats = globalStats[key];
		var rightCount = 0;
		var wrongCount = 0;
		if (stats) {
			rightCount = typeof stats.right === 'number' ? stats.right : 0;
			wrongCount = typeof stats.wrong === 'number' ? stats.wrong : 0;
		}
		var practiceCount = rightCount + wrongCount;
		// Less practiced => higher weight
		var baseWeight = 1 / (1 + practiceCount);
		// Often wrong => extra boost: wrongRate = wrong/total attempts (0..1). Multiply by (1 + 3*wrongRate) so words often wrong get up to 4x more weight than never-wrong at same practice
		var wrongRate = practiceCount > 0 ? wrongCount / practiceCount : 0;
		var wrongBoost = 1 + 3 * wrongRate;
		weights.push(baseWeight * wrongBoost);
	}
	return weights;
}

// Pick a random index from availableIndices with probability proportional to weight at that index.
function x_pickWeightedWordIndex(availableIndices, weights) {
	if (!availableIndices || availableIndices.length === 0) return -1;
	...
	for (var i = 0; i < availableIndices.length; i++) {
		...
		total += (weights[idx] != null && weights[idx] > 0) ? weights[idx] : 1;
	}
	if (total <= 0) return availableIndices[Math.floor(Math.random() * availableIndices.length)];
	var r = Math.random() * total;
	for (var j = 0; j < availableIndices.length; j++) {
		var idx = availableIndices[j];
		var w = (weights[idx] != null && weights[idx] > 0) ? weights[idx] : 1;
		r -= w;
		if (r <= 0) return idx;
	}
...
// Create exercise assignments with random distribution
// Each word should appear exactly 2 times per level, randomly distributed across exercise types.
// wordWeights (optional): array same length as words; higher = prefer (less practiced). If provided, less-practiced words appear more often.
function x_createExerciseAssignments(words, exerciseTypes, totalCount, wordWeights) {
	var assignments = [];
	var wordUsage = {};

	// Initialize word usage counter
	for (var i = 0; i < words.length; i++) {
		wordUsage[i] = 0;
	}

	// Create assignments ensuring each word is used 2 times
	for (var i = 0; i < totalCount; i++) {
		// Find words that haven't been used 2 times yet
		var availableWords = [];
		for (var j = 0; j < words.length; j++) {
			if (wordUsage[j] < 2) {
				availableWords.push(j);
			}
		}

		// If all words used 2 times, reset (shouldn't happen but safety check)
		...
		// Pick word: weighted by practice (less practiced = more often) if wordWeights provided
		var wordIndex = (wordWeights && wordWeights.length === words.length)
			? x_pickWeightedWordIndex(availableWords, wordWeights)
			: availableWords[Math.floor(Math.random() * availableWords.length)];
		var wordData = words[wordIndex];
		wordUsage[wordIndex]++;

		// Pick random exercise type
		var exerciseType = exerciseTypes[Math.floor(Math.random() * exerciseTypes.length)];

		// Note: virtualPageData will be set later in x_generateExercises
		assignments.push({
			exerciseType: exerciseType,
			wordData: wordData,
			config: null // Will be set in x_generateExercises with level info
		});
	}

	// Shuffle assignments for random order
	var shuffled = x_shuffleArray(assignments);
	
	// Ensure no two consecutive exercises of the same type
	return x_avoidConsecutiveExercises(shuffled);
}

// Get exercise configuration based on type and word data
function x_getExerciseConfig(exerciseType, wordData, virtualPageData) {
	var config = {};
	...
	switch (exerciseType) {
		case 'flashCards':
			config = {
				word: wordData.word,
				betekenis: wordData.betekenis,
				contextzin: wordData.contextzinExpliciet
			};
			break;

		case 'meerkeuzevraag':
			// meerkeuzevraag expects:
			// - woordItemCorrect: the correct word
			// - woordItemText: the question text (e.g., "Wat is de betekenis van...?")
			// - woordItemOptions: options (one per line, should include the correct meaning)
			// - woord: the word to speak (optional)
			var word = x_stripHtml(wordData.word || '');
			var contextzin = wordData.contextzinExpliciet ? x_stripHtml(wordData.contextzinExpliciet) : '';
			var questionText = 'Wat is de betekenis van "' + word + '"?';
			if (contextzin) {
				questionText = 'Wat is de betekenis van "' + word + '" in deze zin: ' + contextzin;
			}
			
			// Generate options: correct meaning + 3 random other meanings
			// Strip HTML tags from all options to prevent JavaScript evaluation errors
			var correctMeaning = x_stripHtml(wordData.betekenis || '');
			var options = [correctMeaning];
			if (typeof x_wordListData !== 'undefined' && x_wordListData && x_wordListData.length > 0) {
				var otherWords = x_wordListData.filter(function(w) {
					return w.word !== wordData.word && w.betekenis && w.betekenis.trim() !== '';
				});
				if (otherWords.length >= 3) {
					// Shuffle and take 3 random meanings
					var shuffled = otherWords.sort(function() { return Math.random() - 0.5; });
					for (var i = 0; i < 3 && i < shuffled.length; i++) {
						if (shuffled[i].betekenis && shuffled[i].betekenis.trim() !== '') {
							options.push(x_stripHtml(shuffled[i].betekenis));
						}
					}
				} else if (otherWords.length > 0) {
					// Use all available meanings
					for (var i = 0; i < otherWords.length; i++) {
						if (otherWords[i].betekenis && otherWords[i].betekenis.trim() !== '') {
							options.push(x_stripHtml(otherWords[i].betekenis));
						}
					}
				}
			}
			...
			config = {
				...
				woordItemText: questionText,
				woordItemOptions: options.join('\n'), // Options separated by newlines
				woord: word
			};
			break;

		case 'woordMemory':
			// Level 1: 2 pairs (4 cards total), Level 2: 3 pairs (6 cards total)
			var numberOfPairs = 2; // Default for level 1 (4 cards total)
			if (virtualPageData && virtualPageData.level === 2) {
				numberOfPairs = 3; // 6 cards total for level 2
			}
			config = {
				words: [wordData.word],
				meanings: [wordData.betekenis],
				numberOfPairs: numberOfPairs
			};
			break;

		case 'herhaalDeZin':
			// Level 1: show word, correct answer = word
			// Level 2: show explicit meaning, correct answer = word
			// Level 3: show implicit meaning, correct answer = word + 4 word choices
			var level = virtualPageData ? virtualPageData.level : 1;
			var showMeaning = 'false';
			var betekenisToShow = '';
			
			if (level === 2) {
				// Level 2: show explicit meaning (betekenis)
				showMeaning = 'true';
				betekenisToShow = wordData.betekenis || '';
			} else if (level === 3) {
				// Level 3: show implicit meaning (contextzinImpliciet)
				showMeaning = 'true';
				betekenisToShow = wordData.contextzinImpliciet || wordData.betekenis || '';
			}
			// Level 1: showMeaning remains false, meaning we show the word
			
			config = {
				zin1: wordData.word, // Always store the word as zin1 (correct answer)
				betekenis1: betekenisToShow, // What to show when showMeaning is true
				showMeaning: showMeaning,
				level: String(level) // So HTML can show 4 word options on level 3
			};
			break;

		case 'woordverdwijner':
			// Create imageHtml with only the word to display (no context sentence)
			var wordDisplay = x_stripHtml(wordData.word);
			var imageHtml = '<p><span>' + wordDisplay + '</span></p>';
			config = {
		...
		case 'woordtrainerFillBlanks':
			// woordtrainerFillBlanks requires explicit example sentence - no fallback
			if (!wordData.contextzinExpliciet || wordData.contextzinExpliciet.trim() === '') {
				// Return null to skip this exercise
				return null;
			}
			
			// Check if the word appears exactly (case-insensitive) in the explicit sentence
			var word = x_stripHtml(wordData.word || '').trim();
			var explicitSentence = x_stripHtml(wordData.contextzinExpliciet || '').trim();
			if (!word || !explicitSentence) {
				return null;
			}
			
			// Check if word appears exactly in sentence (case-insensitive word boundary match)
			var wordRegex = new RegExp('\\b' + word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'i');
			if (!wordRegex.test(explicitSentence)) {
				// Word doesn't appear exactly in the sentence, skip this exercise
				return null;
			}
			
			// Level 3 uses typing mode
			var typingMode = 'false';
			if (virtualPageData && virtualPageData.level === 3) {
				typingMode = 'true';
			}
			
			// For multiple choice mode (not typing), generate 3 random distractors from word list
			var distractors = [];
			if (typingMode === 'false' && typeof x_wordListData !== 'undefined' && x_wordListData && x_wordListData.length > 0) {
				// Get other words (excluding the current word)
				var otherWords = x_wordListData.filter(function(w) {
					return w.word && w.word.trim() !== '' && w.word !== wordData.word;
				});
				...
				if (otherWords.length >= 3) {
					var shuffled = otherWords.sort(function() { return Math.random() - 0.5; });
					for (var i = 0; i < 3 && i < shuffled.length; i++) {
						distractors.push(x_stripHtml(shuffled[i].word));
					}
				} else if (otherWords.length > 0) {
					// Use all available words if less than 3
					for (var i = 0; i < otherWords.length; i++) {
						distractors.push(x_stripHtml(otherWords[i].word));
					}
				}
			}
			
			config = {
				text: 'Vul het ontbrekende woord in.',
				sentence: wordData.contextzinExpliciet,
				correctWord: wordData.word,
				typingMode: typingMode,
				distractors: distractors.join(',') // Join as comma-separated string for the XML attribute
			};
			break;

		case 'woordtrainerQuiz':
			// woordtrainerQuiz expects:
			// - woordItemCorrect: the correct word
			// - woordItemText: the question text
			// - woordItemOptions: options separated by |
			// - woordItemImageHtml: HTML content to display (meaning as text)
			var word = x_stripHtml(wordData.word || '');
			var betekenis = x_stripHtml(wordData.betekenis || '');
			
			// Generate options: correct word + 3 random other words
			var options = [word];
			if (typeof x_wordListData !== 'undefined' && x_wordListData && x_wordListData.length > 0) {
				var otherWords = x_wordListData.filter(function(w) {
					return w.word && w.word.trim() !== '' && w.word !== wordData.word;
				...
				if (otherWords.length >= 3) {
					...
					var shuffled = otherWords.sort(function() { return Math.random() - 0.5; });
					for (var i = 0; i < 3 && i < shuffled.length; i++) {
						if (shuffled[i].word && shuffled[i].word.trim() !== '') {
							options.push(x_stripHtml(shuffled[i].word));
						}
					}
				} else if (otherWords.length > 0) {
					// Use all available words
					for (var i = 0; i < otherWords.length; i++) {
						if (otherWords[i].word && otherWords[i].word.trim() !== '') {
							options.push(x_stripHtml(otherWords[i].word));
						}
					}
				}
			}
		...
		case 'associaties':
			config = {
				word: wordData.word,
				betekenis: wordData.betekenis,
				contextzin: wordData.contextzinImpliciet || wordData.contextzinExpliciet
			};
			break;
		...
		case 'woordzoeker':
			config = {
				correctWord: wordData.word,
				contextzin: wordData.contextzinExpliciet || wordData.contextzinImpliciet
			};
			break;

		case 'woordtrainerQuizSpelling':
			// Level 3 uses typing mode - always true for woordtrainerQuizSpelling since it's only used in level 3
			var typingMode = 'true'; // woordtrainerQuizSpelling is only used in level 3, so always typing mode
			
			var word = x_stripHtml(wordData.word || '');
			var betekenis = x_stripHtml(wordData.betekenis || '');
			var contextzinExpliciet = x_stripHtml(wordData.contextzinExpliciet || '');
			var contextzinImpliciet = x_stripHtml(wordData.contextzinImpliciet || '');
			
			// woordspellingAudio.html reads woordItemCorrect, not word
			config = {
				woordItemCorrect: word,
				word: word, // Also set word for compatibility
				betekenis: betekenis,
				contextzin: contextzinExpliciet || contextzinImpliciet,
				contextzinExpliciet: contextzinExpliciet,
				contextzinImpliciet: contextzinImpliciet,
				typingMode: typingMode,
				level: '3' // Set level so HTML can detect it
			};
			break;
	}
...
// Avoid consecutive exercises of the same type
function x_avoidConsecutiveExercises(assignments) {
	if (assignments.length <= 1) {
		return assignments;
	}
	
	var result = assignments.slice();
	var maxAttempts = 200; // Prevent infinite loops
	var attempts = 0;
	
	// Try to fix consecutive duplicates
	while (attempts < maxAttempts) {
		var hasConsecutive = false;
		
		// Check for consecutive exercises of the same type
		for (var i = 0; i < result.length - 1; i++) {
			if (result[i].exerciseType === result[i + 1].exerciseType) {
				hasConsecutive = true;
				
				// Try to find a different exercise type to swap with (prefer later positions)
				var foundSwap = false;
				for (var j = i + 2; j < result.length; j++) {
					if (result[j].exerciseType !== result[i].exerciseType) {
						// Swap positions
						var temp = result[i + 1];
						result[i + 1] = result[j];
						result[j] = temp;
				...
				// If no swap found in later positions, try earlier positions
				if (!foundSwap && i > 0) {
					for (var j = i - 1; j >= 0; j--) {
						if (result[j].exerciseType !== result[i].exerciseType && 
						    (j === 0 || result[j - 1].exerciseType !== result[i].exerciseType)) {
							// Swap positions (only if it won't create a new consecutive duplicate)
							var temp = result[i + 1];
							result[i + 1] = result[j];
							result[j] = temp;
							foundSwap = true;
				...
				// If still no swap found, try a more aggressive shuffle of remaining items
				if (!foundSwap) {
					// Shuffle from current position onwards
					var remaining = result.slice(i + 1);
					var shuffled = x_shuffleArray(remaining);
					for (var k = 0; k < shuffled.length; k++) {
						result[i + 1 + k] = shuffled[k];
					}
				}
		...
		if (!hasConsecutive) {
			break; // No consecutive duplicates found
		}
		
		attempts++;
		
		// If we've tried many times, do a full reshuffle as last resort
		if (attempts === maxAttempts - 1) {
			result = x_shuffleArray(assignments);
...
// Get virtual page data (deprecated - use direct index access)
function x_getVirtualPage(index) {
	if (index >= 0 && index < x_virtualPages.length) {
		return x_virtualPages[index];
	}
	return null;
}

// Check if level is completed and navigate to completion page if needed
function x_checkLevelCompletion() {
	if (!x_autoGenerateMode || !x_virtualPages || x_virtualPages.length === 0) {
		return;
	}

	try {
		// Find completion pages and check if their level is completed
		for (var i = 0; i < x_virtualPages.length; i++) {
			var virtualPage = x_virtualPages[i];
			if (virtualPage.type === 'levelCompletion' && virtualPage.config) {
				var level = virtualPage.config.level;
				var levelStartIndex = virtualPage.config.levelStartIndex;
				var levelEndIndex = virtualPage.config.levelEndIndex;
				...
				if (completedCount >= totalExercises) {
					...
					// Check if we're currently on the last exercise of this level
					var currentVirtualIndex = x_currentPage - baseIndex;
					if (currentVirtualIndex === levelEndIndex && currentVirtualIndex >= 0 && currentVirtualIndex < x_virtualPages.length) {
						// Navigate to completion page after a short delay
						setTimeout(function() {
							x_changePage(completionPageIndex);
						}, 1500); // 1.5 second delay to show success feedback first
					}
				}
			}
		}
...
// Count completed exercises in a level range
function x_countCompletedExercisesInLevel(startIndex, endIndex) {
	try {
		...
		for (var i = startIndex; i <= endIndex && i < x_virtualPages.length; i++) {
			...
			try {
				const doneData = JSON.parse(localStorage.getItem(donePrefix) || '{}');
				if (doneData[exerciseKey]) {
					completedCount++;
				}
			} catch(e) {}
		}
...
// Get the current level based on the current page index
function x_getCurrentLevel(pageIndex) {
	if (!x_autoGenerateMode || typeof x_virtualPages === 'undefined' || !x_virtualPages || x_virtualPages.length === 0) {
		return null;
	}
	...
	try {
		...
		// Check if we're before virtual pages (intro, word list, title) - these are part of Level 1
		if (pageIndex < baseIndex) {
			return 1; // Intro, woordenlijst, and title pages are part of Level 1
		}
		
		// Check if we're on a virtual page
		if (pageIndex >= baseIndex && pageIndex < baseIndex + x_virtualPages.length) {
			var virtualIndex = pageIndex - baseIndex;
			if (virtualIndex >= 0 && virtualIndex < x_virtualPages.length) {
				var virtualPage = x_virtualPages[virtualIndex];
				if (virtualPage) {
					return virtualPage.level || null;
				}
			}
		}
...
// Get the level start index (first exercise page to count for progress) for a given level
// Returns x_pageInfo index
function x_getLevelStartIndex(level) {
	if (!x_autoGenerateMode || typeof x_virtualPages === 'undefined' || !x_virtualPages || x_virtualPages.length === 0) {
		return null;
	}
	...
	try {
		...
		// Find the levelCompletion virtual page for this level
		for (var i = 0; i < x_virtualPages.length; i++) {
			var vp = x_virtualPages[i];
			if (vp.type === 'levelCompletion' && vp.config && vp.config.level === level) {
				// levelStartIndex in config might be stored as x_pageInfo index or virtual page index
				// Check by comparing with baseIndex - if it's >= baseIndex, it's an x_pageInfo index
				// Otherwise, it's a virtual page index
				if (typeof vp.config.levelStartIndex === 'number') {
					var configStartIndex = vp.config.levelStartIndex;
					// If config value is >= baseIndex, it's already an x_pageInfo index
					if (configStartIndex >= baseIndex) {
						return configStartIndex;
					} else {
						// It's a virtual page index, convert to x_pageInfo index
						return baseIndex + configStartIndex;
					}
				}
				
				// Search from the start up to the completion page to find the earliest exercise for this level
				for (var j = 0; j < i; j++) {
					if (x_virtualPages[j] && x_virtualPages[j].level === level && x_virtualPages[j].type !== 'levelCompletion') {
						return baseIndex + j;
					}
				}
				// Fallback to the completion page index if none found
				return baseIndex + i;
			}
		}
		return null;
...
// Get the level end index (last exercise page to count for progress) for a given level
// Returns x_pageInfo index
function x_getLevelEndIndex(level) {
	if (!x_autoGenerateMode || typeof x_virtualPages === 'undefined' || !x_virtualPages || x_virtualPages.length === 0) {
		return null;
	}
	...
	try {
		...
		for (var i = 0; i < x_virtualPages.length; i++) {
			var vp = x_virtualPages[i];
			if (vp.type === 'levelCompletion' && vp.config && vp.config.level === level) {
				// levelEndIndex in config might be stored as x_pageInfo index or virtual page index
				// Check by comparing with baseIndex - if it's >= baseIndex, it's an x_pageInfo index
				// Otherwise, it's a virtual page index
				if (typeof vp.config.levelEndIndex === 'number') {
					var configEndIndex = vp.config.levelEndIndex;
					// If config value is >= baseIndex, it's already an x_pageInfo index
					if (configEndIndex >= baseIndex) {
						return configEndIndex;
					} else {
						// It's a virtual page index, convert to x_pageInfo index
						return baseIndex + configEndIndex;
					}
				}
				
				// Fallback: use the page right before the completion page (last exercise)
				if (i > 0) {
